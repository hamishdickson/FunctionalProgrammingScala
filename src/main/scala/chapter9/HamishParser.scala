...
/**
  * Let's create a Parser! Hamish Stylee
  *
  ...
  */
trait HamishParser[ParseError, Parser[+_]] { self =>
  /**
  ...
  /**
    * We want a parser that can (say) take a list of Chars with zero or many 'a's in there and return the count of 'a's
    *
    * implementation? this kind of looks like it needs a map
    */
  def zeroOrMany[A](p: Parser[A]): Parser[Int]

  /**
    * Same as above, but with one or many. If there are zero, then it should do something - what should it do?
    */
  def oneOrMany[A](p: Parser[A]): Parser[Int]
